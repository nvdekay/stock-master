    ...
    const handleStatusUpdate = async (shipmentId, newStatus) => {
        try {
            ...
            if (newStatus === 'delivered') {
                await ApiService.patch(`/orders/${shipment.orderId}`, { status: 'completed' });
            }

            setShipments((prev) =>
                prev.map((s) =>
                    s.id === shipmentId
                        ? { ...s, status: newStatus, deliveryDate: newStatus === 'delivered' ? new Date().toISOString().split('T')[0] : s.deliveryDate }
                        : s
                )
            );
